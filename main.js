...
intrductionSection.style.display="none"
...
document.getElementById("headline").innerText="BRYCE FILES"

document.getElementById("countdown").style.display="none"
document.getElementById("decryptSection").style.display="none"
document.getElementById("content").style.display="block"
                intrductionSection.style.display="block"
                videoSection.style.display="none"
                videoSection2.style.display="none"
                intrductionVideo.play().catch(e => console.error("Error playing introduction video:", e));
                releaseSound.play().catch(e => console.error("Error playing release sound:", e));
...
const intrductionVideo = document.getElementById("intrduction")
const intrductionSection = document.getElementById("intrductionSection")
const releaseVideoElement = document.getElementById("releaseVideo")
...
if(intrductionVideo && intrductionSection && releaseVideoElement && glitchSound && bennettGlitchText && secondVideo && videoSection && videoSection2 && thanksScreen && statement && bennettScreen && realEvidenceBtn && realEvidenceScreen){

    intrductionVideo.addEventListener("ended", () => {
        intrductionSection.style.display = "none";
        videoSection.style.display = "block";
        releaseVideoElement.play().catch(e => console.error("Error playing release video:", e));
    });

    releaseVideoElement.addEventListener("ended", () => {
        videoSection.style.display = "none";
        videoSection2.style.display = "block";
        secondVideo.play().catch(e => console.error("Error playing second video:", e));
    });
...
secondVideo.addEventListener("ended",()=>{
        ...
        setTimeout(()=>{
            ...
            realEvidenceBtn.addEventListener("click",()=>{
                ...
                realEvidenceVideo.play().catch(e => console.error("Error playing real evidence video:", e));

                // Add listener for realEvidenceVideo ending
                realEvidenceVideo.addEventListener("ended", () => {
                    realEvidenceScreen.style.display = "none";
                    thanksScreen.querySelector('h2').innerText = "Thanks For Participating";
                    thanksScreen.style.display = "flex"; // Use flex for centering
                    // Close page after 8 seconds
                    setTimeout(() => {
                        window.close();
                    }, 8000);
                });
